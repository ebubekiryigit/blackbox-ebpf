...
Bump the application build version in `internal/version/VERSION` only. Capture and
socket protocol discriminators in `model` currently prevent incompatible decoding;
they are not a published compatibility policy before the first public release.
...
The pre-release YAML format has no public version or migration history. Do not add
one until the maintainer defines the first published compatibility policy.
...
Preserve independent capture fixtures. Define and document public upgrade and
compatibility policy before the first release rather than inventing migrations now.
